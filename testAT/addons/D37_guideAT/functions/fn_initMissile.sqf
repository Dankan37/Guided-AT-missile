...
//Add the action to the whole crew
{
	...
	_id = _x addAction ["Missile FOV", {
		...
		[_projectile, _offset, [], _interface] spawn D37AT_fnc_handleMissile;
...
if(isTouchingGround _unit) then {
	[_projectile, _unit] spawn {
		...
		private _targetAlt = _altUnit + 150;
		...
		_id = ["D37_handleMissileLevel", "onEachFrame", {
